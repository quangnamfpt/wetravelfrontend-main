...
function RegisterPassword({ languageSelected, handleNextScreen, role, setPasswordMain }) {
    ...
    return (<div className='container form-register-info'>
        ...
        {role == 3 ?
            ...
            : <Link onClick={handleCheckPasswordRegister} to={`/register-profile-partner?email=${email}`} className='text-decoration-none btn-next-register-info'>{languageList[3]} <HiOutlineArrowNarrowRight className='space-left' /></Link>}
    </div>)
}
